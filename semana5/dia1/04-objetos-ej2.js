let resultados = {
   ...
   //let nombre =prompt("Ingresar nombre");
       for (let i = 0; i < resultados.locations.length; i++) {
          ...
          { console.log(resultados.locations[i])
           break;}
           if(i >= resultados.locations.length - 1) 
           {console.log("No se encontro la ciudad");}
       }
   }
   buscarPorNombre("Wintefall");

   const mostrarEstadistica=()=>{
       arreglo = [];
    for (let i = 0; i < resultados.locations.length; i++) {
        let infectados = +resultados.locations[i].latest.confirmed;
        let poblacion = +resultados.locations[i].country_population
        let porcentaje = infectados*100/poblacion;
        if(porcentaje > 0.001)
        {
            arreglo.push(resultados.locations[i].country);
        }
    }
    return arreglo;
   }

  let paises = mostrarEstadistica();
  console.log(paises);