...
let dibujar = (arregloPaises) => {
    //modificar el html interno con jq
    $("#cuerpo").html("");
...
$("#busqueda").keyup(e => {
    ...
    let resultados = paises.filter((pais) => {
        if (pais.country.toLowerCase().indexOf(busqueda.toLowerCase()) >= 0) {
            return pais;
        }
        
    });
    dibujar(resultados);
    instance.mark(busqueda);
});